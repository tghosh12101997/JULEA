...
static void _benchmark_object_create(BenchmarkRun* run, gboolean use_batch) {
    guint const n = 1000;
    g_autoptr(JBatch) delete_batch = j_batch_new(j_benchmark_get_semantics());
    g_autoptr(JBatch) batch = j_batch_new(j_benchmark_get_semantics());
    gboolean ret;
    ...
    for (guint i = 0; i < n; i++) {
        ...
        g_autoptr(JObject) object = j_object_new("benchmark", name);
        j_object_create(object, batch);
        j_object_delete(object, delete_cdels auto exec = !use_batch;
...
static void benchmark_object_create(BenchmarkRun* run) {
    _benchmark_object_create(run, FALSE);
}

static void benchmark_object_create_batch(BenchmarkRun* run) {
    _benchmark_object_create(run, TRUE);
}

static void _benchmark_object_delete(BenchmarkRun* run, gboolean use_batch) {
    guint const n = 1000;
    g_autoptr(JBatch) batch = j_batch_new(j_benchmark_get_semantics());
    gboolean ret;
    ...
    for (guint i = 0; i < n; i++) {
        gchar* name = g_strdup_printf("benchmark-%d", i);
        g_autoptr(JObject) object = j_object_new("benchmark", name);
        j_object_create(object, batch); // Objects must be created before they can be deleted in this context
...
static void benchmark_object_delete(BenchmarkRun* run) {
    _benchmark_object_delete(run, FALSE);
}

static void benchmark_object_delete_batch(BenchmarkRun* run) {
