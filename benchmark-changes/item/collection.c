...
{
    guint const n = 1000; // This could be parameterized if different runs need different sizes
    ...
    run->timings = g_array_new(FALSE, FALSE, sizeof(gdouble)); // Ensure timings are initialized
    ...
    while (j_benchmark_iterate(run))
    {
        ...
        for (guint i = 0; i < n; i++)
        {
            ...
            gdouble elapsed = g_timer_elapsed(op_any_timer, NULL);
            g_timer_destroy(op_timer);
            g_array_append_val(run->timings, elapsed);
...
{
    guint const n = 1000; // Number of collections
    ...
    while (j_benchmark_iterate(run))
    {
        ...
        for (guint i = 0; i < n; i++)
        {
            ...
            gdouble elapsed = g_timer_elapsed(op_timer, NULL);
            g_timer_destroy(op_timer);
            g_array_append_val(run->timings, elapsed);
