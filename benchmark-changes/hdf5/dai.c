 ...
 * Copyright (C) 2020-2024 Michael Kuhn
 *
...
{
    JSemantics* semantics = j_benchmark_get_semantics();
    j_hdf5_set_semantics(semantics);
    j_semantics_unref(semantics); // Ensure proper reference handling
}
...
static gboolean deserialize_attribute_data(gconstpointer value, guint32 len, gconstpointer* data, gsize* data_size)
{
    bson_t bson[1];
    ...
    {
        gchar const* key = bson_iter_key(&iterator);

        if (g_strcmp0(key, "type") == 0 && bson_iter_int32(&iterator) != 3)
        {
            return FALSE;
        }
...
static void write_attribute(hid_t group, gchar const* name)
{
    hsize_t dims[1] = {1024};
    int data[1024];
    ...
    hid_t attribute = H5Acreate2(group, name, H5T_NATIVE_INT, dataspace, H5P_DEFAULT, H5P_DEFAULT);
    H5Awrite(attribute, H5T_NATIVE_INT, data);
    H5Sclose(dataspace);
    H5Aclose(attribute);
}

static void read_attribute(hid_t group, gchar const* name)
{
...
static void benchmark_hdf_dai_native(BenchmarkRun* run)
{
    guint const n = 25;
    guint iter = 0;
    ...
    while (j_benchmark_iterate(run))
    {
        ...
        for (guint i = 0; i < n; i++)
        {
            gchar* name = g_strdup_printf("benchmark-dai-native-%u.h5", i + (iter * n));
            hid_t file = H5Fcreate(name, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
            hid_t group = H5Gcreate2(file, "benchmark-dai-native", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

            for (guint j = 0; j < n * 10; j++)
            {
                gchar* aname = g_strdup_printf("benchmark-dai-native-%u", j);
                g_timer_start(op_timer); // Start timing the write operation
                write_attribute(group, aname);
                gdouble elapsed_time = g_timer_elapsed(op_timer, NULL);
                g_array_append_val(run->timisms, elapsed_time); // Record the time
                g_free(aname);
            }
