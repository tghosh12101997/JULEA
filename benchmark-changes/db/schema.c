...
_benchmark_db_schema_create(BenchmarkRun* run, gboolean use_batch)
{
    ...
    while (j_benchmark_iterate(run))
    {
        ...
        for (guint i = 0; i < n; i++)
        {
            ...
            g_autofree gchar* name = g_strdup_printf("benchmark-schema-%d", i);
            schema = j_db_schema_new("benchmark-ns", name, &error);

            for (guint j = 0; j < 10; j++)
            {
                g_autofree gchar* fname = g_strdup_printf("field%d", j);
...
static void
benchmark_db_share_delete_batch(BenchmarkRun* run)
{
...
benchmark_db_schema(void)
{
    ...
    j_benchmark_add("/db/schema/delete-batch", benchmark_db_share_delete_batch);
}
