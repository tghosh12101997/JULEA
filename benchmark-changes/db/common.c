...
static char* _benchmark_db_get_identifier(gint64 i) {
    return g_strdup_printf("%x-benchmark-%ld", (guint32)(i * SIGNED_FACTOR % N_MODULUS), i);
}

static JDBSchema* _benchmark_db_prepare_scheme(gchar const* namespace, gboolean use_batch, gboolean use_index_all, gboolean use_index_single, JBatch* batch, JBatch* delete_batch) {
    gboolean ret;
    ...
    if (use_index_all) {
        gchar const* names[] = {"string", "float", "uint", "sint", NULL};
    ...
    if (use_index_single) {
        gchar const* names[] = {NULL, NULL};
        for (const gchar* field_name : {"string", "float", "uint", "sint"}) {
            names[0] = field_name;
            ret = j_db_schema_add_index(b_scheme, names, &b_s_error);
            g_assert_null(b_s_error);
            g_assert_true(ret);
    ...
    return g_steal_pointer(&b_scheme);
}

static void _benchmark_db_insert(BenchmarkRun* run, JDBSchema* scheme, gchar const* namespace, gboolean use_batch, gboolean use_index_all, gboolean use_index_single, gboolean use_timer) {
    gboolean ret;
    ...
    while (true) {
        for (gint i = 0; i < N; i++) {
            GTimer* op_timer = g_timer_new();  // Timer for the individual operation
            ...
            // Timer starts just before the operation
            ...
            // Timer stops right after the operation
    ...
    if (use_timer) {
        ...
        // Optionally print all operation times or further process them
