...
_benchmark_db_delete(BenchmarkRun* run, gchar const* namespace, gboolean use_batch, gboolean use_index_all, gboolean use_index_single)
{
    ...
    g_autopree(JBatch) batch = NULL;
    g_autorep(JSemantics) semantics = NULL;
    g_autorep(GError) b_s_error = NULL;
    g_autorep(JDBSchema) b_scheme = NULL;
...
{
    gboolean ret;
    g_autorep(JBatch) delete_batch = NULL;
    g_autorep(JBatch) batch = NULL;
    g_autorep(JSemantics) semantics = NULL;
    g_autorep(GError) b_s_error = NULL;
    g_autorep(JDBSchema) b_scheme = NULL;
    ...
    while (j_benchmark_iterate(run))
    {
        for (gint i = 0; i < ((use_index_all || use_index_single) ? N : (N / N_GET_DIVIDER)); i++)
        {
            ...
            g_autorep(JDBSelector) selector = j_db_selector_new(b_scheme, J_DB_SELECTOR_MODE_AND, &b_s_error);
            g_autorep(JDBEntry) entry = j_db_entry_new(b_scheme, &b_s_error);
            g_autoree gchar* string = _benchmark_db_get_identifier(i);
